...
class habit_tracker:
    ...
    def DHabits(self):
        ...
        for rd in r:
            if count % 2 == 0:
                my_tree.insert("",'end',iid=count,text='',values=(rd[0],rd[2],rd[3],rd[4],rd[5],rd[6],rd[7],rd[8],rd[9],rd[10]), 
                        tags=('evenrow',))
            else:
                my_tree.insert("",'end',iid=count,text='',values=(rd[0],rd[2],rd[3],rd[4],rd[5],rd[6],rd[7],rd[8],rd[9],rd[10]), 
                        tags=('oddrow',))
        ...
        def habit_completion():
            ...
            c.execute("SELECT count(*) FROM habit_completion hc WHERE hc.habit_id = "+id_entry.get()+
                      " AND substr(hc.completion_date, 1, 10)=substr('"+currentDateTime+"', 1, 10)")
            ...
            if int(habit_completion_count) == 1:
                ...
                c.execute("INSERT INTO habit_completion(habit_id, completion_status, completion_date) VALUES(:habit_id, :completion_status, :completion_date)",
                            {
        ...
        def longest_streak():
            ...
            try:
                ...
                longest_streak_qry = longest_streak_qry+ " AND hb.users_id = "+str(user_id)+ " AND hb.habit_status='A' AND hc.completion_status=1 GROUP BY hc.habit_id HAVING count(*) > 1 ORDER BY count(*) desc)"
        ...
        def last_month_struggle():
            ...
            last_month_struggle_qry = last_month_struggle_qry+ " AND hb.users_id = "+str(user_id)+ " AND hb.habit_status='A' AND hc.completion_status=1 "
            ...
            conn.close()
            #display the message box to the user
            messagebox.showinfo(title="Last month struggle",message="User had the struggle with the following habit \n"+myhabitstreak, parent=DH_nxt)    
        ...
        def current_habits():
            ...
            try:
                current_habits_qry = "SELECT hb.habit_name FROM habit hb WHERE hb.users_id = "+str(user_id)+" AND habit_status='A' ORDER BY hb.id"
        ...
        def add_habit():
            ...
            def save_habit():
                ...
                try:
                    ...
                    messagebox.showinfo(title="Info",message="Sun, Mon, Tue, Wed, Thu, Fri and Sat entries must be 1 or 0!", parent=habit)
                except ValueError:
                    messagebox.showinfo(title="Info",message="Please select anyone of the day ( Enter 1 in any one of the Day )!", parent=habit)

                    
            # create a save button
            save_button = Button(habit, text='Save', width=10, height=2, font='bold', fg='white', bg='green', command=save_habit)
            save_button.place(x = 100, y = 350)
            # create button to close the current window
            habit_exit_button = Button(habit, text='Close', width=10, height=2, font='bold', fg='white', bg='green', command=habit.destroy)
            # add exit button
        ...
        def fill_entries(e):
            ...
            if (int(currentDay) in (sunday, monday, tuesday, wednesday, thursday, friday, saturday)) and values[9]==str('A'):
        ...
        habit_button=Button(button_frame,text="checked-off",font='bold',width=15,height=1,fg='white',bg='green', command=habit_completion, state=DISABLED)
        habit_button.grid(row=0, column=1, padx=10, pady=10)

        #button to delete a record
        delete_button=Button(button_frame,text="Delete Record",font='bold',width=15,height=1,fg='white',bg='green', command=delete_record, state=DISABLED)
        delete_button.grid(row=0, column=2, padx=10, pady=10)

        #button to add a new habit
        exit_button=Button(button_frame,text="Add new Habit",font='bold',width=15,height=1,fg='white',bg='green',command = add_habit)
        exit_button.grid(row=0, column=3, padx=10, pady=10)

        #button to close the current window
        exit_button=Button(button_frame,text="Close",font='bold',width=15,height=1,fg='white',bg='green',command = DH_nxt.destroy)
        exit_button.grid(row=0, column=4, padx=10, pady=10)

        #button to referesh the current window
        exit_button=Button(button_frame,text="Refresh",font='bold',width=15,height=1,fg='white',bg='green',command = refresh_curr_win)
        ...
        #button to set complete
        habit_button1=Button(rep_button_frame,text="Longest streak",font='bold',width=15,height=1,fg='white',bg='green', command=longest_streak)
        habit_button1.grid(row=0, column=1, padx=10, pady=10)

        #button to show the current habits
        current_habits=Button(rep_button_frame,text="Current Habits",font='bold',width=15,height=1,fg='white',bg='green', command=current_habits)
        current_habits.grid(row=0, column=2, padx=10, pady=10)

        #button to show with which habits did I struggle most last month?
        struggle_button=Button(rep_button_frame, text="last month struggle",font='bold',width=15,height=1,fg='white',bg='green',command = last_month_struggle)
    ...
    def tracker(self):
        ...
        habit_button = Button(root, text='Show Habits', font='bold', width=10, height=1, fg='white', bg='green', command = self.DHabits)
        # add button to the window
        habit_button.place(x = 100, y = 350)

        # create button to close the current window
        exit_button = Button(root, text='Exit', width=10, height=1, font='bold', fg='white', bg='green', command=root.destroy)
        # add exit button
